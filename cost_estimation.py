...
def total_reactor_cost(
    ...
    cost_kt = np.sqrt(f_k*mean_k2 + f_t*mean_t2) + np.sqrt(f_k*max_k2 + f_t*max_t2) # cost due to curvature and torsion
    cost = baseline * (c_v * (vol/vol0)**1.1 + c_coils * (c_l*np.abs(total_current_length/l0)**1.2 + c_kt*(cost_kt/kt0)**1.4) + c_p * P_ext/p0)
    return cost
